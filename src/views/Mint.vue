<template>
  <div class="w-1/2 max-w-sm p-4 bg-black rounded-md bg-opacity-80">
    ...
    <div class="mt-4" v-if="wallet">
      <p>price: {{ (price || 0).toLocaleString() }} SOL</p>
...
const refreshBalance = async () => {
  if (wallet && wallet?.value?.publicKey !== undefined) {
    balance.value =
      (await connection.getBalance(wallet.value.publicKey)) / LAMPORTS_PER_SOL;
  }
};
...
const mint = async () => {
  try {
    ...
    if (
      wallet &&
      candyMachine.value?.program &&
      wallet?.value?.publicKey !== undefined
    ) {
